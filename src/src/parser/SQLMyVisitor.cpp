...
void ParseResult::print(bool output_mode, int duration) {
    if (output_mode == false) {
        for (int i = 0; i < columns.size() - 1; i++) {
            std::cout << columns[i].columnName << ",";
        }
        if (columns.size() > 0)
            std::cout << columns[columns.size() - 1].columnName << std::endl;

        for (auto& item : data_items) {
            for (int i = 0; i < columns.size() - 1; i++) {
                for (int j = 0; j < item.columnIds.size(); j++) {
                    if (item.columnIds[j] == columns[i].columnId) {
                        item.values[j].print();
                        std::cout << ",";
                        break;
                    }
            ...
            for (int i = 0; i < item.columnIds.size(); i++) {
                if (columns.size() > 0 &&
                    item.columnIds[i] ==
                        columns[columns.size() - 1].columnId) {
                    item.values[i].print();
                    break;
                }
            }
        ...
        for (const auto& column : columns) {
            columnNames.push_back(column.columnName);
        }
        ...
        for (const auto& item : data_items) {
            ...
            for (const auto& column : columns) {
                auto it = std::find(item.columnIds.begin(),
                                    item.columnIds.end(), column.columnId);
                if (it != item.columnIds.end()) {
                    int index = std::distance(item.columnIds.begin(), it);
                    rowData.push_back(item.values[index].toString());
                } else {
                    rowData.push_back("");
                }
            }
    ...
    if (primary_keys.size() + foreign_keys.size() > 0) {
        ...
        for (auto& foreign_key : foreign_keys) {
            ...
            for (int i = 0; i < foreign_key.fKColumnNames.size() - 1;
                 i++) {
                std::cout << foreign_key.fKColumnNames[i] << ", ";
            }
            std::cout << *std::next(
                             foreign_key.fKColumnNames.begin(),
                             foreign_key.fKColumnNames.size() - 1)
            ...
            std::cout << *std::next(
                    ...
                    foreign_key.reference_column_names.size() - 1) << ");" << std::endl;
        }
    }
    if (indexes.size() > 0) {
        std::cout << std::endl;
...
std::any SQLMyVisitor::visitUse_db(antlr4::SQLParser::Use_dbContext* ctx) {
    ...
    return sm -> setActiveDatabase(ctx->Identifier()->getText().c_str());
}
...
std::any SQLMyVisitor::visitDelete_from_table(
    ...
    condition::updateAllIndexConditions(index_conditions, conditions, sm,
                                          std::vector<std::string>{table_name});

    std::vector<system::SearchConstraint> constraints;
    for (auto& index_condition : index_conditions) {
        constraints.push_back(index_condition.toSearchConstraint());
    }
    ...
    return sm->deleteRowsFromTable(table_id, constraints);
}
...
std::any SQLMyVisitor::visitUpdate_table(
    ...
    sm->getColumnID(table_id, column_names, data_item.columnIds);

    return sm->updateRows(table_id, constraints, data_item);
}

std::any SQLMyVisitor::visitSelect_table_(antlr4::SQLParser::Select_table_Context* ctx) {
    return visitChildren(ctx);
}
...
std::any SQLMyVisitor::visitSelect_table(
    ...
    if (table_names.size() == 1) {
        ...
        if (! sm -> searchRowsInTable(table_id, constraints, result_datas, column_types,
                        record_locations, -1)) {
            return false;
        }

        if (order_by_column_name != "") {
            int order_columnId = -1;
            for (auto& column_type : column_types) {
                if (column_type.columnName == order_by_column_name) {
                    order_columnId = column_type.columnId;
                    break;
                }
            }
            if (order_columnId == -1) {
            ...
            // sort result_datas accordto it's item's datavlue[order_columnId]
            std::sort(
                result_datas.begin(), result_datas.end(),
                [order_columnId, order_ascending](const record::DataItem& a,
                                                   const record::DataItem& b) {
                    if (order_ascending)
                        return a.values[order_columnId] <
                               b.values[order_columnId];
                    else
                        return a.values[order_columnId] >
                               b.values[order_columnId];
                });
    ...
    for (auto& index_condition : index_conditions) {
        if (index_condition.type == condition::VariableType::TABLE_COLUMN) {
            ...
            join_edges[point_id_1][point_id_2].start_pt_id.push_back(
                index_condition.columnId);
            join_edges[point_id_1][point_id_2].end_pt_id.push_back(
                index_condition.columnIdOther);
            ...
            table_id2seaerch_constraint[table_id].push_back(
                index_condition.toSearchConstraint());
        }
    }
    ...
    std::map<std::pair<int, int>, int> original_table_columnId2new_columnId;
    std::map<int, std::pair<int, int>> new_columnId2original_table_columnId;
    int new_columnId_cnt = 0;
    ...
    for (int round = 0; round < total_table_num; round++) {
        ...
        if (round == 0) {
            for (auto& column_type : upp_column_types) {
                original_table_columnId2new_columnId[std::make_pair(
                    select_table_id, column_type.columnId)] =
                    new_columnId_cnt;
                new_columnId2original_table_columnId[new_columnId_cnt] =
                    std::make_pair(select_table_id, column_type.columnId);
                column_type.columnId = new_columnId_cnt;
                new_columnId_cnt++;
            ...
            while (low_page <= total_page) {
                ...
                for (auto& data_item : upp_result_datas) {
                    for (int i = 0; i < data_item.columnIds.size(); i++) {
                        data_item.columnIds[i] =
                            original_table_columnId2new_columnId
            ...
            int sort_columnId_for_selected_table = -1;
            int sort_table_id_for_previous_result = -1;
            int sort_original_columnId_for_previous_result = -1;
            ...
            for (int edge_point_id = 0; edge_point_id < total_table_num;
                ...
                if (sort_columnId_for_selected_table == -1) {
                    sort_columnId_for_selected_table =
                        edge_from_selected_table[edge_point_id].start_pt_id[0];
                    sort_table_id_for_previous_result = table_id;
                    sort_original_columnId_for_previous_result =
            ...
            if (sort_columnId_for_selected_table != -1) {
                int sort_columnIdx_for_previous_result =
                    original_table_columnId2new_columnId[std::make_pair(
                        sort_table_id_for_previous_result,
                        sort_original_columnId_for_previous_result)];

                // sort result_datas by sort_columnIdx_for_previous_result
                std::sort(
                    result_datas.begin(), result_datas.end(),
                    [sort_columnIdx_for_previous_result](
                ...
                int sort_columnIdx_for_selected_table = -1;
                for (int i = 0; i < select_column_types.size(); i++) {
                    if (select_column_types[i].columnId ==
                        sort_columnId_for_selected_table) {
                        sort_columnIdx_for_selected_table = i;
                        break;
                    }
                }
                if (sort_columnIdx_for_selected_table == -1) {
                    std::cout << "!ERROR" << std::endl;
                    std::cout << "Column " << sort_columnId_for_selected_table
                ...
                while (low_page <= total_page) {
                    ...
                    // sort_columnIdx_for_selected_table
                    std::sort(
                    ...
                    while (select_result_datas_idx <
                        ...
                        while (
                            ...
                            select_result_datas[select_result_datas_idx]
                                    .values
                                        [sort_columnIdx_for_selected_table] <
                                result_datas[result_datas_idx].values
                                    [sort_columnIdx_for_previous_result])
                            select_result_datas_idx++;
                        ...
                        while (
                            ...
                            result_datas[result_datas_idx_end].values
                                    [sort_columnIdx_for_previous_result] ==
                                result_datas[result_datas_idx_begin].values
                                    [sort_columnIdx_for_previous_result])
                        ...
                        for (; select_result_datas_idx_begin <
                            ...
                            for (int i = result_datas_idx_begin;
                                ...
                                for (auto& joint_constraint :
                                    ...
                                    for (int j = 0;
                                         j <
                                         select_result_data.columnIds.size();
                                         j++) {
                                        if (select_result_data.columnIds[j] ==
                                            start_pt_id) {
                                            start_pt_id_idx = j;
                                            break;
                                        }
                                    ...
                                    for (int j = 0;
                                         j < result_data.columnIds.size();
                                         j++) {
                                        if (result_data.columnIds[j] ==
                                            end_pt_id) {
                                            end_pt_id_idx = j;
                                            break;
                                        }
                                    ...
                                    if (select_result_data
                                            .values[start_pt_id_idx] !=
                                        result_data
                                            .values[end_pt_id_idx]) {
                                        flag = false;
                                        break;
                                    }
                                }
                                if (flag) {
                                    ...
                                    for (int j = 0;
                                         j <
                                         select_result_data.values.size();
                                         j++) {
                                        new_data.values.push_back(
                                            select_result_data.values[j]);
                                        new_data.columnIds.push_back(
                                            original_table_columnId2new_columnId
                                                [std::make_pair(
                                                    select_table_id,
                                                    select_result_data
    ...
    for (auto& item : column_names) {
        ...
        sm->getColumnID(table_id, {column_name}, columnIds);
        int columnId = columnIds[0];

        int new_id = original_table_columnId2new_columnId[std::make_pair(
            table_id, columnId)];

        if (output_mode == true) {
            auto insert_column_type = column_types[new_id];
            insert_column_type.columnName = table_name + "." + column_name;
...
std::any SQLMyVisitor::visitAlter_add_index(
    ...
    return sm->addIndex(table_name.c_str(), index_name, columnIds, false);
}
...
std::any SQLMyVisitor::visitAlter_table_add_pk(
    ...
    sm->addPrimaryKey(table_id, columnIds);
    return true;
}
...
std::any SQLMyVisitor::visitAlter_table_add_foreign_key(
    ...
    for (auto& column_name : reference_column_names) {
        ...
        for (int i = 0; i < reference_column_types.size(); i++) {
            if (reference_column_types[i].columnName == column_name) {
                columnId = reference_column_types[i].columnId;
                break;
            }
        }
        if (columnId == -1) {
        ...
        reference_columnIds.push_back(columnId);
    }

    system::ForeignKeyInfo fk_info(columnIds, reference_table_id,
                                   reference_columnIds, constraint_name);
...
std::any SQLMyVisitor::visitAlter_table_add_unique(
    ...
    std::vector<record::ColumnType> column_types;
    sm->getTableColumnTypes(table_id, column_types);
    std::vector<int> unique_columnIds;
    for (auto& column_name : unique_column_names) {
        ...
        unique_columnIds.push_back(columnId);
    }

    // TODO: add unique constraint to *table_id* - *unique_columnIds*
...
std::any SQLMyVisitor::visitNormal_field(
    ...
    if (not_node != nullptr && ctx->Null() != nullptr) {
        columnType.isNotNull = true;
    }

    columnType.columnName = ctx->Identifier()->getText();
...
std::any SQLMyVisitor::visitForeign_key_field(
    ...
    enum class Expecting {
        ...
        REFERENCE_COLUMN_NAMES,
        FINISHED
    } expecting_target = Expecting::fKColumnNames;

    for (auto child : ctx->children) {
        std::any result = child->accept(this);
        switch (expecting_target) {
            case Expecting::fKColumnNames:
                if (result.type() == typeid(std::vector<std::string>)) {
                    for (auto& res :
                         std::any_cast<std::vector<std::string>>(result)) {
                        fKColumnNames.push_back(res);
                    }
    ...
    if (fKColumnNames.size() != reference_column_names.size()) {
        std::cout << "!ERROR" << std::endl;
    ...
    foreign_key.fKColumnNames = fKColumnNames;
...
std::any SQLMyVisitor::visitType_(antlr4::SQLParser::Type_Context* ctx) {
    ...
    if (type == "INT") {
        return_val.dataType = record::DataTypeIdentifier::INT;
    } else if (type == "FLOAT") {
        return_val.dataType = record::DataTypeIdentifier::FLOAT;
    } else if (type == "DATE") {
        return_val.dataType = record::DataTypeIdentifier::DATE;
    } else if (type.find("VARCHAR") != std::string::npos) {
        return_val.dataType = record::DataTypeIdentifier::VARCHAR;
        return_val.varcharLength = std::stoi(type.substr(8, type.size() - 9));
    } else {
        std::cout << "!ERROR" << std::endl;
...
std::any SQLMyVisitor::visitValue_list(
    ...
    for (auto child : ctx->children) {
        ...
        if (res.type() == typeid(std::string)) {
            ...
            result.push_back(record::DataValue(record::DataTypeIdentifier::INT, false,
                                               std::any_cast<int>(res)));
        } else if (res.type() == typeid(double)) {
            result.push_back(record::DataValue(record::DataTypeIdentifier::FLOAT,
                                               false,
                                               std::any_cast<double>(res)));
        } else if (res.type() == typeid(record::DateValue)) {
            result.push_back(
                record::DataValue(record::DataTypeIdentifier::DATE, false,
        ...
        switch (expecting_target) {
            ...
            case EXPRESSION:
                ...
                if (result.type() == typeid(std::string)) {
                    std::get<1>(assignment).dataType =
                        record::DataTypeIdentifier::VARCHAR;
                    std::get<1>(assignment).value.charValue =
                        std::any_cast<std::string>(result);
                    std::get<1>(assignment).isNull = false;
                } else if (result.type() == typeid(int)) {
                    std::get<1>(assignment).dataType =
                        record::DataTypeIdentifier::INT;
                    std::get<1>(assignment).value.intValue =
                        std::any_cast<int>(result);
                    std::get<1>(assignment).isNull = false;
                } else if (result.type() == typeid(double)) {
                    std::get<1>(assignment).dataType =
                        record::DataTypeIdentifier::FLOAT;
                    std::get<1>(assignment).value.floatValue =
                    ...
                    std::get<1>(assignment).dataType =
                        record::DataTypeIdentifier::ANY;
                    std::get<1>(assignment).isNull = true;
                } else {
                    // TODO: DateType
                    std::cout << "!ERROR" << std::endl;
