...
class RecordManager {
   public:
    ...
    // upd 新接口↓
    void updateColumnUnique(const char* file_path, int columnId, bool unique);
    /**
     ...
     * insert顺序可以不按照column_types的顺序，只需要保证data_item的columnId和column_types的columnId一致
     *
     * @param file_path 文件路径
   ...
   private:
    ...
    const int columnCacheCapacity = 10;
};
